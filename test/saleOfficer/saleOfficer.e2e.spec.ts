...
let id = '';
describe('Almuhasba Sale Officer Test Cases', () => {
  ...
  it(`Should give error on Less than 3 character firstName`, async () => {
    const userTest = await helper.saleOfficer();
  ...
  it(`Get All saleOfficer `, async () => {
    return request(app.getHttpServer())
      .get('/saleOfficer?column=id')
      .set('Authorization', `${token}`)
      .expect(200);
  });
  it(`Should Add saleOfficer `, async () => {
    const userTest = await helper.saleOfficer();
    return request(app.getHttpServer())
      .post('/saleOfficer')
      .set('Authorization', `${token}`)
  ...
  it(`Should Update saleOfficer `, async () => {
    const userTest = await helper.saleOfficer();
    userTest.lastName = 'NEW Updated';
    return request(app.getHttpServer())
      .patch('/saleOfficer/' + id)
      .set('Authorization', `${token}`)
      .send(userTest)
      .expect(204);
  });
  it(`Should Delete saleOfficer by Id`, async () => {
    return request(app.getHttpServer())
      .delete('/saleOfficer/' + id)
      .set('Authorization', `${token}`)
      .expect(204);
  });
  afterAll(async () => {
